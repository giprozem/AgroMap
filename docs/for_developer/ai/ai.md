...
- `ProcessAdmin`: ⚙️ **Admin settings for the Process model**.
- `Merge_BandsAdmin`: 🌀 **Admin settings for the Merge_Bands model**.
- `Create_RGBAdmin`: 🌈 **Admin settings for the Create_RGB model**.
- `Cut_RGB_TIFAdmin`: ✂️🖼️ **Admin settings for the Cut_RGB_TIF model**.
- `AI_FoundAdmin`: 🤖🔍 **Admin settings for the AI_Found model**.
- `CreateDescriptionAdmin`: ✏️📝 **Admin settings for the CreateDescription model**.
...
- `Images_AIAdmin`: 🖼️ **Admin settings for the Images_AI model** with an HTML-rendered photo view.
...
- `Process`: ⚙️ **Singleton model representing a single process or task**. Contains attributes like the running state and process number.
...
- `Cut_RGB_TIF`: ✂️🖼️ **Singleton model representing the process of cropping color images in TIFF format**. Contains attributes like completion state and process number.
...
- `CreateDescription`: ✏️📝 **Singleton model storing instructions for creating a dataset**. Contains a textual description field.

### 📄 [/ai/models/predicted_contour.py](/ai/models/predicted_contour.py)

- `Images_AI`: 🖼️ **Model to store image files used for AI-related processes**. Supports image uploads.
...
- Custom Methods:
  ...
  - `get_district`: 🏞️ Method to get the district ID based on the polygon using geospatial queries.
  - `get_db_district`: 🏙️ Method to get the district ID from the database based on the `conton` relationship.
...
- `CreateAPIView`: 📦 **API view for creating datasets**. This view is accessible only to admin users and provides the ability to start a dataset creation process.
  ...
  - Methods:
    - `get`: ✉️ Handles GET requests to check the dataset creation process status and initiate a new process if it's not running. Also, it sends notifications when a new dataset is created.
...
- `predicted_contour()`: 🕵️‍♀️ **Contour Prediction Function**
...
- `clean_contour_and_create_district()`: 🏞️ **Contour Cleaning and District Creation Function**
