            ...
            for veg_index in VegetationIndex.objects.all():
                ...
                try:
                    ...
                    if veg_index.name == 'NDVI':
                        ...
                        result_to_save = ndvi_calculator(B04=output_path_b04, B08=output_path_b8a, saving_file_name=file_name)

                    elif veg_index.name == 'NDMI':
                        average_value = average_ndmi(swir_file=output_path_b11, nir_file=output_path_b8a)

                        result_to_save = ndmi_calculator(B11=output_path_b11, B08=output_path_b8a, saving_file_name=file_name)

                    elif veg_index.name == 'NDWI':
                        average_value = average_ndwi(green_file=output_path_b03, nir_file=output_path_b8a)

                        result_to_save = ndwi_calculator(B03=output_path_b03, B08=output_path_b8a, saving_file_name=file_name)
                    elif veg_index.name == 'NDRE':
                        average_value = average_ndre(red_file=output_path_b07, nir_file=output_path_b8a)

                        result_to_save = ndre_calculator(B07=output_path_b07, B8A=output_path_b8a, saving_file_name=file_name)
                    elif veg_index.name == 'SAVI':
                        average_value = average_savi(red_file=output_path_b04, nir_file=output_path_b8a)

                        result_to_save = savi_calculator(B04=output_path_b04, B08=output_path_b8a, saving_file_name=file_name)
                    ...
                    print(f'exception {contour.id}')
                    with open('report.txt', 'a') as file_report:
                        file_report.write(f"{contour.id}' = f'{e}")
                        file_report.write(',')
                        file_report.write('\n')
