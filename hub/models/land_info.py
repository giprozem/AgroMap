...
class LandInfo(BaseModel):
    ...
    crop_yield = models.DecimalField(decimal_places=2, max_digits=100, blank=True, null=True, verbose_name='Урожайность')
    property_type = models.ForeignKey(PropertyTypeList, blank=True, null=True, on_delete=models.CASCADE, related_name='land_info', verbose_name='данные о собственности')
    document_type = models.ForeignKey(DocumentTypeList, blank=True, null=True, on_delete=models.CASCADE, related_name='land_info', verbose_name='правоустанавливающие документы')
    document_link = models.FileField(upload_to='document', blank=True, null=True, verbose_name='Ссылка на документ')
    category_type = models.ForeignKey(CategoryTypeList, blank=True, null=True, on_delete=models.CASCADE, related_name='land_info', verbose_name='категории земель')
    land_type = models.ForeignKey(LandTypeList, blank=True, null=True, on_delete=models.CASCADE, related_name='land_info', verbose_name='данные по типам угодьев')
    square = models.DecimalField(decimal_places=2, max_digits=100, blank=True, null=True, verbose_name='Фактическая площадь')
    """Zem Balance"""
    ...
    territorial_outline = models.CharField(max_length=100, blank=True, null=True, verbose_name='Административно-территориальное деление')
    number_realestateunits = models.CharField(max_length=100, blank=True, null=True, verbose_name='Количество единиц недвижимости')
    ...
    certifying_act = models.CharField(max_length=55, blank=True, null=True, verbose_name='Удостоверяющий акт (прикрепить файл)')
    use_end = models.CharField(max_length=55, blank=True, null=True, verbose_name='Дата завершения пользования')
    agroland_purposes = models.CharField(max_length=55, blank=True, null=True, verbose_name='Земли сельскохозяйственного назначения')
    gfsu = models.CharField(max_length=255, blank=True, null=True, verbose_name='Государственный фонд сельскохозяйственных угодий')
    ...
    agro_type_land = models.CharField(max_length=255, blank=True, null=True, verbose_name='Сельскохозяйственный вид угодий')
    ...
    collective_gardens_and_veg = models.CharField(max_length=255, blank=True, null=True, verbose_name='Коллективные сады и огороды')
    ...
    improved_radical = models.CharField(max_length=3, blank=True, null=True, verbose_name='Улучшенные (коренного улучшения)')
    inaccessible = models.CharField(max_length=3, blank=True, null=True, verbose_name='Неиспользуемый из-за труднодоступности')
    ...
    plant_not_forestfund = models.CharField(max_length=255, blank=True, null=True, verbose_name='Древесно кустарниковые насаждения, не входящие в государственный лесной фонд')
    ...
    elementary_sectionnumber = models.CharField(max_length=55, blank=True, null=True, verbose_name='Номер элементарного участка')
    asr_address = models.CharField(max_length=100, blank=True, null=True, verbose_name='Адрес участка (АСР)')
    special_purpose_asr = models.CharField(max_length=100, blank=True, null=True, verbose_name='Целевое назначение (АСР)')
    ...
    land_factarea_asr = models.CharField(max_length=255, blank=True, null=True, verbose_name='Фактическая площадь (АСР)')
    form_using_asr = models.CharField(max_length=55, blank=True, null=True, verbose_name='Форма использования (АСР)')
    haushold_land = models.CharField(max_length=55, blank=True, null=True, verbose_name='Приусадебные земли и служебные наделы граждан')
