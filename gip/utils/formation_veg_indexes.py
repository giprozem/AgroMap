...
def signals_veg_index_creating(satellite_image, creating_report_obj, veg_index_obj, contour_obj):
    """
...
def create_veg_indexes(contour):
    if contour.type.name_en in ['Аgricultural land', 'Pasture', 'Cropland']:
        start_time = time.time()
        satellite_images = SciHubImageDate.objects.filter(date__year=contour.year)
        ...
        print(f"create_veg_indexes_contour_obj - Processing completed in {elapsed_time:.2f} seconds")
